...
class ic_settings_search {
	...
	function placeholder() {
		if ( isset( $_GET[ 'tab' ] ) && $_GET[ 'tab' ] === 'help' ) {
			...
			$placeholder = __( 'Search for help', 'ecommerce-product-catalog' );
		}
		return $placeholder;
	}
	...
	function search_html() {
		...
		<form class="ic-settings-search<?php echo $class ?>">
			...
			foreach ( $_GET as $get_key => $get_value ) {
				...
				?>
				<input type="hidden" name="<?php echo esc_attr( sanitize_text_field( $get_key ) ) ?>" value="<?php echo esc_attr( sanitize_text_field( $get_value ) ) ?>">
				<?php
			}
			?>
			<input type="search" name="ic-settings-search" placeholder="<?php echo $placeholder ?>" value="<?php echo esc_attr( $search_word ) ?>">
	...
	function search_results_settings( $search_word ) {
		...
		?>
		<h2><?php echo sprintf( __( "%s adjustment options found in settings for '%s'", 'ecommerce-product-catalog' ), count( $this->points ), $search_word ) ?>:</h2>
		<ul>
			<?php
			...
			foreach ( $this->points as $option_name => $number ) {
				...
				?>
				<li class="ic-settings-search-result <?php echo $class ?>"><a href="<?php echo esc_url( $url ) ?>"><?php echo $label ?></a><?php echo $additional ?></li>
				<?php
				$num++;
			}
			if ( count( $this->points ) > 5 ) {
				?>
				<li><div class="button-secondary ic-search-settings-show-button"><?php echo sprintf( __( 'Show %s more related adjustment options', 'ecommerce-product-catalog' ), count( $this->points ) - 5 ) ?></div></li>
				<?php
			}
			?>
		</ul>
	...
	function ajax_search_docs() {
		$search_word = isset( $_POST[ 'term' ] ) ? $_POST[ 'term' ] : '';
		if ( empty( $search_word ) ) {
			return '';
		}
		$transient_name		 = 'ic-cat-search-docs-' . sanitize_title( $search_word );
		...
		if ( !empty( $data ) && is_array( $data ) ) {
			ob_start();
			?>
			<h2><?php echo sprintf( __( "%s documentation pages found for '%s'", 'ecommerce-product-catalog' ), count( $data ), $search_word ) ?>:</h2>
			<ul>
				<?php
				foreach ( $data as $key => $doc ) {
					...
					?>
					<li class="ic-docs-search-result<?php echo $class ?>"><a href="<?php echo esc_url( add_query_arg( array( 'cam' => 'settings-search', 'key' => 'doc-link' ), $doc[ 'link' ] ) ) ?>" target="_blank"><?php echo esc_attr( $doc[ 'title' ] ) ?></a><?php echo $additional ?></li>
					<?php
				}

				if ( count( $data ) > 5 ) {
					?>
					<li><div class="button-secondary ic-search-docs-show-button"><?php echo sprintf( __( 'Show %s more related docs', 'ecommerce-product-catalog' ), count( $data ) - 5 ) ?></div></li>
					<?php
				}
				?>
			</ul>
	...
	function js() {
		...
		$option_name = urldecode( $_GET[ 'find_option_name' ] );
		?>
		<script>
		    jQuery( document ).ready( function ( ) {
